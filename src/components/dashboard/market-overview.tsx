...
// Mock data generator for newest coins
...
export default function MarketOverview() {
  ...
  const [sortBy, setSortBy] = useState<"marketCap" | "price" | "holders" | "athMarketCap" | "age">("marketCap")
  ...
  // Safety checks and early return if not mounted
  if (!mounted) {
    return (
      ...
      <div className="space-y-4">
        <div>
          <h2 className="text-lg font-semibold text-foreground mb-2">
            New Coins (Market Cap {(minMarketCap / 1000000).toFixed(0)}M+)
          </h2>
          <p className="text-xs text-muted-foreground">Jump in on promising new tokens - updated every second</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-muted/5 rounded-lg border border-border">
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-2 block">Min Market Cap (M)</label>
            <input
              type="number"
              value={minMarketCap / 1000000}
              onChange={(e) =>
                setAPIKey("minMarketCap", Math.max(1, Number.parseFloat(e.target.value) || 25) * 1000000)
              }
              className="w-full px-3 py-2 bg-background border border-border rounded text-sm text-foreground"
            />
          </div>
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-2 block">Quick Buy Amount</label>
            <div className="flex gap-2">
              <input
                ...
                className="w-12 px-3 py-2 bg-background border border-border rounded text-sm text-foreground"
              />
              <button
        ...
        <div className="space-y-2 max-h-80 overflow-y-auto">
          {sortedCoins.map((coin) => (
            <div key={coin.symbol} className="border border-border rounded-lg p-3 hover:bg-muted/5 transition-colors">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="flex items-start justify-between">
                  <div>
                    <div className="font-semibold text-foreground text-sm">{coin.symbol}</div>
                    <div className="text-xs text-muted-foreground">{coin.name}</div>
                    <div className="text-xs text-muted-foreground mt-1">Age: {getCoinAge(coin.createdAt)}</div>
                  </div>
                  <div className={`text-xs font-medium ${coin.change24h >= 0 ? "text-accent" : "text-red-500"}`}>
                    {coin.change24h >= 0 ? "+" : ""}
                    {coin.change24h}%
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="bg-muted/5 p-2 rounded">
                    ...
                    <div className="text-foreground font-medium">${(coin.marketCap / 1000000).toFixed(1)}M</div>
                  </div>
                  <div className="bg-muted/5 p-2 rounded">
                    <div className="text-muted-foreground">ATH MC</div>
                    <div className="text-foreground font-medium">${(coin.athMarketCap / 1000000).toFixed(1)}M</div>
                  </div>
                  <div className="bg-muted/5 p-2 rounded">
                  ...
                  <div className="bg-muted/5 p-2 rounded">
                    ...
                    <div className="text-foreground font-medium">{(coin.holders / 1000).toFixed(0)}k</div>
                  </div>
                </div>

                <div className="bg-muted/5 p-2 rounded text-xs">
                  <div className="text-muted-foreground">Total Fees</div>
                  <div className="text-foreground font-medium">${(coin.totalFees / 1000).toFixed(1)}k</div>
                </div>
