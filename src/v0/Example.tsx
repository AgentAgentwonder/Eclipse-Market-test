...
export const V0Example: React.FC = () => {
  const [count, setCount] = useV0LocalStorage({
    key: 'v0-example-count',
    defaultValue: 0,
  });
    ...
    <div className="v0-container">
      <V0Card className="max-w-md mx-auto mt-8">
        ...
        <V0CardContent>
          ...
          <p className="mb-4">Count from localStorage: {formatNumber(count)}</p>
          <div className="flex gap-2">
            <V0Button onClick={() => setCount(count + 1)} variant="primary">
              Increment
            </V0Button>
            <V0Button onClick={() => setCount(0)} variant="secondary">
              Reset
            </V0Button>
          </div>
        </V0CardContent>
